...
class JointAttack:
    ...
    def attack(self, img_path, target_path):
        # Soft-label attack
        (target_filename, sl_queries) = self.soft_label.attack(img_path=img_path, target_path=target_path)
        # Hard-label attack
        (adv_img, hl_queries) = self.hard_label.attack(orig_img_path=target_path, 
